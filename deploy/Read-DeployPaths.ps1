# Loads deploy/deploy-paths.json (VPS SSH/SCP targets). Call with -DeployRoot set to the deploy/ folder.
param(
    [Parameter(Mandatory = $true)]
    [string] $DeployRoot,
    [string] $Profile = ""
)
$path = Join-Path $DeployRoot "deploy-paths.json"
...
if ([string]::IsNullOrWhiteSpace($name)) {
    Write-Error "deploy-paths.json: set ActiveDeployProfile or pass -Profile."
}
